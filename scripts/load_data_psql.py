...
Creates the PostgreSQL schema and loads all cleaned CSV data.

Usage:
    python scripts/load_data_psql.py

Prerequisites:
    1. Start the container:  docker start some-postgres
    2. Run cleaning first:   python scripts/clean_data.py
"""
...
# ── schema creation ──────────────────────────────────────────────────────────
CREATE_DB = f"CREATE DATABASE {DB_NAME} ENCODING 'UTF8';"
...
def load_campaigns(conn):
    ...
    # Cast nullable integer columns to Int64 to avoid "146.0" float representation
